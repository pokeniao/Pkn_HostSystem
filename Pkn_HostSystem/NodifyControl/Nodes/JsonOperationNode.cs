...
{
    public partial class JsonOperationNode : PknNode
    {
        [ObservableProperty] private JsonOperationModel model = new();
        public JsonOperationNode(DesignModel designModel, Object model = null) : base(designModel, $"Json字符串处理", NodeEnum.JsonProcessing)
        {
            //从本地读取
            if (model != null)
            {
                JObject? jObject = model as JObject;
                Model = jObject?.ToObject<JsonOperationModel>();
            }

            IModel = Model;
            Operation = new JsonProcessingOperation(this);
