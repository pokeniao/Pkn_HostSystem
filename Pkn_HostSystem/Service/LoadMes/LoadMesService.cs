...
public class LoadMesService
{
    ...
    public string RunUserDefined(string JsonKey, out bool noReturn, out string ErrorMessage)
    {
        switch (JsonKey)
        {
            case "ppp:Base003_OrderList:scheduleCode":
                string scheduleCode = new PppBase003OrderList().DynCurrentOrder("scheduleCode");
                ...
                return scheduleCode;
            case "ppp:Base003_OrderList:orderCode":
                string orderCode = new PppBase003OrderList().DynCurrentOrder("orderCode");
                if (orderCode == null)
                {
                    ErrorMessage = "未选择工单!请选择工单后操作";
                    noReturn = true;
