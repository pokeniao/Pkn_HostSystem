            ...
            if (item.GetMessageType == "通讯")
            {
                ...
                switch (methodName)
                {
                    ...
                    case "读DM寄存器":
                        ...
                        (bool b4, string? responseLateProcess4) = await _self.LateProcess(item, readDm);
                        if (b4)
                        {
                    ...
                    case "串口通讯":
                        ...
                        (bool b, string? response) = await ScpiSerialAsync(item, cts);
                        if (!b)
                        {
