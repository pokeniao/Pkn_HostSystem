    ...
    public class Voc2:IUserDefined
    {
        public async Task<(bool Succeed, object Return)> Main(CancellationTokenSource cts, params object[] args)
        {
            try
            {
                ...
                station1.电芯条码1 = GetString(await PlcModbusTcp.ReadHoldingRegisters_03(1, 80, 20)).Trim('\0').Trim('\n').Trim('\r');
                station1.电芯条码2 = GetString(await PlcModbusTcp.ReadHoldingRegisters_03(1, 100, 20)).Trim('\0').Trim('\n').Trim('\r');
