        ...
        switch (ModbusToolModel.FuntionCode_select)
        {
            ...
            case "06写单寄存器":
                try
                {
                    await ModbusBase.WriteRegister_06((byte)ModbusToolModel.SlaveAddress,
                        (ushort)ModbusToolModel.StartAddress,
                        ushort.Parse(ModbusToolModel.WriteDvgList[0].value.ToString()));
                }
                catch (Exception exception)
                {
                    Log.ErrorAndShow($"读取发生错误:{exception.Message}");
            ...
            case "0F写多线圈":
                try
                {
                    ...
                    foreach (var modbusPojo in ModbusToolModel.WriteDvgList.ToArray())
            ...
            case "10写多寄存器":
                ...
                try
                {
                    foreach (ModbusPojo<object> modbusPojo in ModbusToolModel.WriteDvgList.ToArray())
