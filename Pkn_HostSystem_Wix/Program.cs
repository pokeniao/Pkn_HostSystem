    ...
    public class Program
    {
        static void Main()
        {
            ...
            var project = new Project("Pkn_HostSystem",
                new Dir(@"%LocalAppData%\Pokeniao\Pkn_HostSystem",
                    new Files($@"{baseDir}\*.*")
                ),
            ...
            //无法使用---------vvvvvvvvvvvvvvvvvvvvv-------
            // “当前用户注册表（HKCU）” 的内容放到了一个“系统级安装路径（如 ProgramFiles）” 的组件中，这是 Wix 不允许的，因为这导致“混合安装范围”，可能会造成不可预测的权限问题或行为。
